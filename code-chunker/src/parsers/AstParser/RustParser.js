...
class RustParser extends BaseParser {
    constructor(config) {
        ...
        this.nodeTypes = {
            ...
            type: ['struct_item', 'union_item', 'enum_item', 'trait_item', 'impl_item', 'type_item'],
    ...
    async parseContent(content, filePath = null) {
        try {
            ...
            if (content.length > 10 * 1024 * 1024) {
                console.warn(`Content too large for Rust parsing in file: ${filePath || 'unknown'} (${content.length} bytes)`);
                return [];
            }
            ...
            if (cleanContent.length > 1024 * 1024) {
                console.warn(`Large Rust file detected: ${filePath || 'unknown'} (${cleanContent.length} bytes), truncating for parsing`);
            ...
            const allChunks = [...modules, ...functions, ...types, ...variables, ...macros, ...comments, ...other];
