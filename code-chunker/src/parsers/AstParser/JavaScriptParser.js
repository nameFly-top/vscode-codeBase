...
class JavaScriptParser extends BaseParser {
    ...
    static getMetadata() {
        return {
            ...
            version: '1.0.0',
            description: 'JavaScript/TypeScript语言AST解析器',
            nodeTypes: {
                import: ['import_statement'],
                export: ['export_statement'],
    ...
    async parseContent(content, filePath = null) {
        try {
            if (!content || typeof content !== 'string') {
                console.warn(
                    `Invalid content for JavaScript parsing in file: ${filePath || 'unknown'}`
                );
                return [];
            }
            ...
            if (content.length === 0) {
                console.warn(
                    `Empty content for JavaScript parsing in file: ${filePath || 'unknown'}`
                );
                return [];
            }
            ...
            if (content.length > 10 * 1024 * 1024) {
                console.warn(
                    `Content too large for JavaScript parsing in file: ${filePath || 'unknown'} (${content.length} bytes)`
                );
                return [];
            }
            ...
            if (cleanContent.length > 1024 * 1024) {
                console.warn(
                    `Large JavaScript file detected: ${filePath || 'unknown'} (${cleanContent.length} bytes), truncating for parsing`
                );
            ...
            console.error(
                `Error parsing JavaScript content in file: ${filePath || 'unknown'}:`,
                error
            );
            return [];
        }
    ...
    _extractExports(tree, code) {
        const exports = [];
        this._traverseNodes(tree.rootNode, node => {
            if (this.nodeTypes.export.includes(node.type)) {
                const nodeCode = this._extractNodeCode(code, node.startIndex, node.endIndex);
                exports.push({
                    type: 'export',
                    content: nodeCode,
