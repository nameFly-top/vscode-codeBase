        ...
        for (const [parserName, ParserClass] of Object.entries(this.availableParsers)) {
            try {
                ...
                if (typeof ParserClass.getMetadata === 'function') {
                    ...
                    console.log(
                        `✅ [PluginManager] 成功加载插件: ${parserName} (${metadata.name})`
                    );
                    ...
                    console.log(
                        `⚠️ [PluginManager] ${parserName} 没有getMetadata方法，使用名称推断`
                    );
                ...
                console.warn(
                    `⚠️ [PluginManager] 插件 ${parserName} 加载失败 (tree-sitter依赖可能未安装): ${error.message}`
                );
