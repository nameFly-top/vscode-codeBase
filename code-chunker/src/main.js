...
class CodeChunker {
    ...
    _loadConfig(userConfig) {
        try {
            ...
            console.error('❌ 致命错误：无法加载配置文件:', error.message);
            throw new Error(
                `配置文件加载失败: ${error.message}。请检查 config/config.js 文件是否存在且格式正确。`
            );
        }
    }
    ...
    async processWorkspace(userId, deviceId, workspacePath, token, ignorePatterns = []) {
        ...
        try {
            ...
            const {
                fileList,
                fileContents,
                merkleTree: scanMerkleTree,
                fileHashes,
                scanStats,
            } = await this.fileScanner.scanWorkspace(workspacePath);
            this.performanceAnalyzer.endFileScanning(
                fileList.length,
                scanStats ? scanStats.skippedFiles : 0
            );
            ...
            const chunks = await this.dispatcher.processFilesConcurrently(
                fileObjects,
                this.parserSelector
            );
            ...
            if (this.progressTracker) {
                ...
                this.log(
                    `File Processing Summary: ${fileProgress.completedFiles}/${fileProgress.totalFiles} files completed (${this.progressTracker.getFileProgressPercentage().toFixed(2)}%)`
                );
                this.log(
                    `Chunk Processing Summary: ${finalProgress.completedChunks}/${finalProgress.totalChunks} chunks completed (${finalProgress.successRate.toFixed(2)}%)`
                );
                ...
                // 完成性能分析并生成报告
                this.performanceAnalyzer.endVectorDBOperations(
                    chunks.length,
                    Math.ceil(chunks.length / 10)
                ); // 假设每批10个
                ...
                this.log(
                    `   📄 MD报告: ${this.performanceAnalyzer.reportPath.replace('.json', '.md')}`
                );

                if (performanceReport) {
                    const totalTime = (performanceReport.summary.totalDuration / 1000).toFixed(2);
                    const score =
                        this.performanceAnalyzer._calculatePerformanceScore(performanceReport);

                    this.log(`\n📈 性能概览:`);
                    this.log(`   ⏱️  总处理时间: ${totalTime}秒`);
                    this.log(
                        `   📁 处理文件数: ${performanceReport.summary.processedFiles}/${performanceReport.summary.totalFiles}`
                    );
                    this.log(`   🧩 生成代码块: ${performanceReport.summary.totalChunks}`);
                    this.log(
                        `   🌐 Embedding请求: ${performanceReport.summary.totalEmbeddingRequests}`
                    );
                            ...
                            const icon =
                                bottleneck.impact === 'high'
                                    ...
                                    : bottleneck.impact === 'medium'
                                      ...
                                      : '🟢';
                            this.log(
                                `   ${index + 1}. ${icon} ${bottleneck.phase}: ${bottleneck.description}`
                            );
                        });
                    } else {
                        this.log(`\n✅ 未检测到明显性能瓶颈，运行良好！`);
                    ...
                    if (performanceReport.performance.recommendations.length > 0) {
                        this.log(`\n💡 性能优化建议:`);
                        performanceReport.performance.recommendations
                            .slice(0, 3)
                            .forEach((rec, index) => {
                                const icon =
                                    rec.priority === 'high'
                                        ...
                                        : rec.priority === 'medium'
                                          ...
                                          : '🟢';
                                this.log(
                                    `   ${index + 1}. ${icon} ${rec.category}: ${rec.suggestion}`
                                );
                            });
                    }
                }
            ...
            this.log(
                `Code Chunker completed in ${((endTime - startTime) / 1000).toFixed(2)} seconds`
            );

            // 只有当所有代码块都成功发送时才认为处理成功
            const isSuccess = this.progressTracker ? 
                this.progressTracker.getOverallProgress().successRate >= 100 : 
                true;
            
            if (!isSuccess) {
                this.warn(`处理未完全成功，成功率: ${this.progressTracker.getOverallProgress().successRate.toFixed(2)}%`);
            }
    ...
    async search(query, options = {}) {
        ...
        try {
            ...
            if (searchResults.length !== filteredResults.length) {
                this.log(
                    `搜索结果过滤: 原始${searchResults.length}条 -> 过滤后${filteredResults.length}条 (移除了${searchResults.length - filteredResults.length}条包含"unknown"的结果)`
                );
            }
    ...
    async _cleanAndRecreateCollection(userId, deviceId, workspacePath) {
        ...
        const databaseName =
            this.config.vectorManager?.database?.query?.defaultDatabase || 'vectorservice-test';
        ...
        try {
            const deleteResult = await this.vectorManager.vectorDB.implementation.dropCollection(
                databaseName,
                collectionName
            );
        ...
        try {
            const createResult = await this.vectorManager.vectorDB.implementation.createCollection(
                databaseName,
                collectionName,
                {
                    ...
                    indexes: [
                        ...
                        // 向量索引
                        {
                            ...
                            indexType: 'HNSW',
                            dimension:
                                this.config.vectorManager?.database?.collections?.vectorDimension ||
                                768,
                            metricType:
                                this.config.vectorManager?.database?.collections?.metricType ||
                                'COSINE',
                            params: {
                                M: 16,
                                efConstruction: 200,
        ...
        try {
            const response =
                await this.vectorManager.vectorDB.implementation.listCollections(databaseName);
            const collections = response.data?.collections || [];

            this.log(
                '验证集合列表:',
                collections.map(col => col.collectionName || col.collection || col.name)
            );
