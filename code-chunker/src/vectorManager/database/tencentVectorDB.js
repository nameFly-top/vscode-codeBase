...
class TencentVectorDB {
    ...
    async listDatabases() {
        try {
            ...
            this.logger.info(
                `获取到 ${response.data?.databases ? response.data.databases.length : 0} 个数据库`
            );
            return response;
        } catch (error) {
    ...
    async createCollection(databaseName, collectionName, params = {}) {
        try {
            ...
            const collectionData = {
                ...
                description:
                    params.description ||
                    `Collection created by Node.js client - ${collectionName}`,
    ...
    async listCollections(databaseName) {
        try {
            ...
            this.logger.info(
                `获取到 ${response.data?.collections ? response.data.collections.length : 0} 个集合`
            );
            return response;
        } catch (error) {
    ...
    async upsertDocuments(databaseName, collectionName, documents) {
        try {
            this.logger.info(
                `向集合 ${databaseName}.${collectionName} 写入 ${documents.length} 条数据`
            );
        ...
        try {
            ...
            this.logger.debug(`向量数组格式检查:`, {
                ...
                firstElementType:
                    vectorArray.length > 0
                        ? Array.isArray(vectorArray[0])
                            ? 'array'
                            : typeof vectorArray[0]
                        : 'N/A',
                firstElementLength:
                    vectorArray.length > 0 && Array.isArray(vectorArray[0])
                        ? vectorArray[0].length
                        : 'N/A',
            });
            ...
            const requestBody = {
                ...
                search: {
                    ...
                    retrieveVector:
                        params.retrieveVector !== undefined ? params.retrieveVector : false, // 修正从params获取参数
                },
            };
            ...
            this.logger.info(
                `向量检索完成，返回 ${response.data?.results ? response.data.results.length : 0} 条结果`
            );
            return response;
        } catch (error) {
    ...
    async upsertCodebase(requestId, database, collection, documents, buildIndex = true) {
        try {
            this.logger.info(
                `代码向量存储: ${documents.length} 个文档到 ${database}.${collection}`
            );
            ...
            // 发送到新的代理接口
            const response = await this.makeCodebaseRequest(
                'POST',
    ...
    async batchUpsert(comboKey, vectors) {
        try {
            ...
            const workspacePath =
                this._extractWorkspacePathFromHash(workspaceHash) || '/unknown/workspace';
            ...
            // 使用新的代码库接口
            const result = await this.upsertCodebase(
                requestId,
                databaseName,
                collectionName,
                documents,
    ...
    async _fallbackBatchUpsert(comboKey, vectors) {
        ...
        try {
            ...
            for (let i = 0; i < batches.length; i++) {
                ...
                const batchResult = await this._uploadBatchWithRetry(
                    databaseName,
                    collectionName,
                    batch
                );
    ...
    _validateAndFilterVectors(vectors) {
        ...
        for (let i = 0; i < vectors.length; i++) {
            ...
            if (codeContent.length > maxCodeSize) {
                ...
                codeContent =
                    codeContent.substring(0, halfSize) +
                    '\n\n... [内容截断] ...\n\n' +
                    codeContent.substring(codeContent.length - halfSize);
                this.logger.debug(
                    `向量 ${vector.id} 代码内容截断: ${vector.code.length} -> ${codeContent.length} 字符`
                );
            }
            ...
            if (estimatedTotalSize > maxDocSize) {
                this.logger.warn(
                    `向量 ${vector.id} 文档仍然过大 (${estimatedTotalSize} > ${maxDocSize})，跳过处理`
                );
                continue;
            }
            ...
            try {
                ...
                if (result.success) {
                    ...
                    if (result.message && result.message.includes('current index is not ready')) {
                        this.logger.warn(
                            `索引未准备就绪，等待${attempt * 2}秒后重试 (${attempt}/${maxRetries})`
                        );
                ...
                if (error.response && error.response.status === 400) {
                    this.logger.error(
                        `客户端错误，不重试: ${error.response.data ? JSON.stringify(error.response.data) : error.message}`
                    );
                    throw error;
                }
                ...
                if (attempt < maxRetries) {
                    this.logger.warn(
                        `批量上传失败，${attempt * 2}秒后重试 (${attempt}/${maxRetries}): ${error.message}`
                    );
                    await new Promise(resolve => setTimeout(resolve, attempt * 2000));
                } else {
        ...
        try {
            ...
            if (describeResult.success) {
                ...
                if (existingDimension && existingDimension !== vectorDimension) {
                    this.logger.info(
                        `集合维度不匹配 (现有: ${existingDimension}, 需要: ${vectorDimension})，重新创建集合...`
                    );
                    ...
                    // 创建新集合
                    await this._createCollectionWithFields(
                        databaseName,
                        collectionName,
                        vectorDimension
                    );
    ...
    async _createCollectionWithFields(databaseName, collectionName, vectorDimension) {
        ...
        const createResult = await this.createCollection(
            databaseName,
            collectionName,
            collectionParams
        );
        ...
        while (retryCount < maxRetries) {
            try {
                ...
                if (result.success) {
                    ...
                    this.logger.info(
                        `集合 ${collectionName} 索引构建完成，耗时 ${Date.now() - startTime}ms`
                    );
                    return;
                }
            } catch (error) {
    ...
    async search(queryVector, topK, comboKey, options = {}) {
        try {
            ...
            const response = await this.searchVectors(
                databaseName,
                collectionName,
                queryVector,
                params
            ...
            if (response.success && response.data) {
                ...
                if (response.data && response.data.code) {
                    ...
                    switch (errorCode) {
                        case 15171:
                            this.logger.warn(
                                `向量维度不匹配: ${errorMsg} - 请检查向量维度是否与集合定义一致`
                            );
                            break;
                        case 14000:
                            this.logger.warn(
                                `字段不存在错误: ${errorMsg} - 请检查outputFields中指定的字段是否在集合中定义`
                            );
                            break;
                        default:
    ...
    async queryDocuments(databaseName, collectionName, filter, params = {}) {
        try {
            ...
            this.logger.info(
                `精确查询完成，返回 ${response.data?.documents ? response.data.documents.length : 0} 条结果`
            );
            return response;
        } catch (error) {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (error.response) {
                    ...
                    if (isBusinessError) {
                        this.logger.warn(
                            `业务警告 (${error.response.status}): ${error.response.data.msg || error.message}`
                        );
                        return {
                            success: false,
                            status: error.response.status,
                        ...
                        this.logger.error(
                            `请求失败 (尝试 ${attempt}/${maxRetries}): ${error.message}`
                        );
                    ...
                    if (isRetryableError && attempt < maxRetries) {
                        ...
                        this.logger.warn(
                            `网络错误，${delay}ms后重试 (${attempt}/${maxRetries}): ${error.message}`
                        );
