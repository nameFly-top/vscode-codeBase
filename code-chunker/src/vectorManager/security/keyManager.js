...
class KeyManager {
    ...
    async _loadMasterKey() {
        ...
        try {
            ...
            if (this.secureStorage) {
                ...
                if (keyData.length < this.keyLength) {
                    this.masterKey = Buffer.concat([
                        keyData,
                        Buffer.alloc(this.keyLength - keyData.length),
                    ]);
            ...
            if (this.masterKey.length !== this.keyLength) {
                throw new Error(
                    `Invalid key length: ${this.masterKey.length} (expected ${this.keyLength})`
                );
            }
        } catch (error) {
    ...
    _startKeyRotation() {
        ...
        if (this.keyRotationInterval < 60000) {
            this.logger.warn(
                `Key rotation interval too short: ${this.keyRotationInterval}ms, setting to 1 hour`
            );
