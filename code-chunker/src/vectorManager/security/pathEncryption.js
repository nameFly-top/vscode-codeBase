...
class PathEncryption {
    ...
    encryptPath(originalPath) {
        ...
        try {
            ...
            this.logger.debug(
                `Encrypted path: ${originalPath.length} chars -> ${encryptedPath.length} chars`
            );
    ...
    decryptPath(encryptedPath) {
        ...
        try {
            ...
            if (
                !encryptedSegments ||
                !encryptedSegments.segments ||
                !Array.isArray(encryptedSegments.segments)
            ) {
            ...
            this.logger.debug(
                `Decrypted path: ${encryptedPath.length} chars -> ${originalPath.length} chars`
            );
    ...
    async _validateEncryptionSetup() {
        try {
            ...
            if (decrypted !== testPath) {
                throw new Error(
                    `Encryption/decryption validation failed: ${decrypted} !== ${testPath}`
                );
            }
