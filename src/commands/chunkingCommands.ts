...
export function registerChunkingCommands(context: vscode.ExtensionContext) {
    ...
    // 注册代码分块命令
    const chunkCodeCommand = vscode.commands.registerCommand(
        'test-electron-treesitter.chunkCode',
        async () => {
            try {
                ...
                vscode.window.showErrorMessage(
                    `代码分块执行失败: ${error instanceof Error ? error.message : String(error)}`
                );
            }
        }
    );
    ...
    // 注册进度查询命令
    const checkProgressCommand = vscode.commands.registerCommand(
        'test-electron-treesitter.checkProgress',
        async () => {
            try {
                ...
                vscode.window.showErrorMessage(
                    `进度查询失败: ${error instanceof Error ? error.message : String(error)}`
                );
            }
        }
    );
    ...
    const clearCacheCommand = vscode.commands.registerCommand(
        'test-electron-treesitter.clearCache',
        async () => {
            try {
    ...
    const cacheStatsCommand = vscode.commands.registerCommand(
        'test-electron-treesitter.cacheStats',
        async () => {
            try {
    ...
    // 注册清除索引缓存命令
    const clearIndexCacheCommand = vscode.commands.registerCommand(
        'test-electron-treesitter.clearIndexCache',
        async () => {
            try {
                ...
                vscode.window.showErrorMessage(
                    `清除索引缓存失败: ${error instanceof Error ? error.message : String(error)}`
                );
            }
        }
    );
    ...
    // 注册网络性能分析报告命令
    const networkPerformanceCommand = vscode.commands.registerCommand(
        'test-electron-treesitter.networkPerformance',
        async () => {
            try {
                ...
                if (report) {
                    ...
                    vscode.window.showInformationMessage(
                        '暂无网络性能数据，请先执行一些代码分块操作。'
                    );
                }
            } catch (error) {
                console.error('[CodeChunker] 生成网络性能报告失败:', error);
                vscode.window.showErrorMessage(
                    `生成网络性能报告失败: ${error instanceof Error ? error.message : String(error)}`
                );
            }
        }
    );
    ...
    // 注册清除网络性能数据命令
    const clearNetworkDataCommand = vscode.commands.registerCommand(
        'test-electron-treesitter.clearNetworkData',
        async () => {
            try {
                ...
                vscode.window.showErrorMessage(
                    `清除网络性能数据失败: ${error instanceof Error ? error.message : String(error)}`
                );
            }
        }
    );
    ...
    const checkIndexStatusCommand = vscode.commands.registerCommand(
        'test-electron-treesitter.checkIndexStatus',
        async () => {
            try {
                const status = await chunkingService.checkIndexStatus(context);
                if (status) {
                    vscode.window.showInformationMessage(
                        `索引状态: ${status.isCompleted ? '✅ 已完成' : '⚠️ 未完成'}`
                    );
                } else {
                    vscode.window.showInformationMessage('索引缓存未初始化或工作区未打开');
                }
            } catch (error) {
                console.error('[CodeChunker] 检查索引状态失败:', error);
                vscode.window.showErrorMessage(
                    `检查索引状态失败: ${error instanceof Error ? error.message : String(error)}`
                );
            }
        }
    );
    ...
    context.subscriptions.push(
        ...
        networkPerformanceCommand,
        clearNetworkDataCommand,
