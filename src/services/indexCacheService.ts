...
export class IndexCacheService {
    ...
    async filterUnindexedFiles(
        ...
        for (const file of files) {
            ...
            try {
                const isIndexed = await this.isFileIndexed(
                    fullPath,
                    workspacePath,
                    userId,
                    deviceId
    ...
    private async calculateFileHash(filePath: string): Promise<string> {
        try {
            ...
            return crypto
                .createHash('md5')
                .update(`${filePath}_${stats.mtime.getTime()}`)
                .digest('hex');
        }
    }
    ...
    private async loadCache(): Promise<void> {
        try {
            ...
            // 检测是否是压缩数据（gzip格式通常以H4sI开头的base64）
            if (cacheContent.startsWith('H4sI') || cacheContent.includes('H4sIAAAAA')) {
                console.warn('[IndexCacheService] 检测到压缩格式的缓存数据，清空缓存重新开始');
                // 删除损坏的缓存文件
                await fs.promises.unlink(this.cacheFilePath).catch(() => {});
                this.cache.clear();
                return;
            }
            ...
            try {
                ...
                console.warn('[IndexCacheService] 缓存文件格式无效，清空缓存重新开始');
                // 删除损坏的缓存文件
                await fs.promises.unlink(this.cacheFilePath).catch(() => {});
                this.cache.clear();
                return;
            ...
            if (!cacheData.version || cacheData.version !== '1.0') {
                console.warn('[IndexCacheService] 缓存版本不兼容，清空缓存重新开始');
                this.cache.clear();
                return;
            }
            ...
            console.log(`[IndexCacheService] 成功加载 ${this.cache.size} 个缓存记录`);
        } catch (error) {
            ...
            try {
                ...
                console.log('[IndexCacheService] 已删除损坏的缓存文件');
            } catch (unlinkError) {
                // 忽略删除失败的错误
            }
        }
