        ...
        await vscode.window.withProgress(
            ...
            async progress => {
                try {
                    ...
                    // 获取chunker实例
                    const chunkerInstance = codeChunker.getChunkerInstance(
                        userId,
                        deviceId,
                        workspacePath,
                        token
                    ...
                    if (
                        !chunkerInstance.vectorManager.vectorDB ||
                        !chunkerInstance.vectorManager.vectorDB.implementation
                    ) {
                    ...
                    // 显示搜索结果
                    await SearchResultView.displaySearchResults(
                        searchQuery,
                        searchResults,
                        workspaceName
                    );
        ...
        await vscode.window.withProgress(
            ...
            async progress => {
                try {
                    ...
                    // 删除collection
                    const result =
                        await chunkerInstance.vectorManager.vectorDB.implementation.dropCollection(
                            databaseName,
                            collectionName
                        );
                    ...
                    try {
                        const collections =
                            await chunkerInstance.vectorManager.vectorDB.implementation.listCollections(
                                databaseName
                            );
                        const collectionExists =
                            ...
                            collections.collections.some(
                                (col: any) => col.collectionName === collectionName
                            );
                    ...
                    vscode.window.showErrorMessage(
                        `删除Collection失败: ${error instanceof Error ? error.message : String(error)}`
                    );
                }
            }
        );
